...
Input: s = "0110", k = 2
Output: false
Explanation: The binary code "00" is of length 2 and doesn't
exist in the array.
...
class Solution:
    def hasAllCodes(self, s: str, k: int) -> bool:
        """
        we put all substrings of length k into a set
        We then check if the set has length 2**k
        """
        
        set_of_substrings = set()
        for start in range(0, len(s) - k + 1):
            set_of_substrings.add(s[start:start+k])
        return len(set_of_substrings) == 2**k
