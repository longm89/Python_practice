...
class Solution:
    def myAtoi(self, s: str) -> int:
        ...
        for char in s[first_digit_pos:]:
            if ord('0') <= ord(char) and ord(char) <= ord('9'):

                if sign == 1 and\
                            (2**31 - 1 - (ord(char)-ord('0')))/10 < final_sum:
                    final_sum = 2**31 - 1
                    break

                if sign == -1 and\
                        (-2**31 + (ord(char) - ord('0')))/10 > - final_sum:
                    # we will multiply with the sign in the end
                    final_sum = 2**31
                    break

                final_sum = final_sum * 10 + (ord(char)-ord('0'))
            else:
                break

        return sign * final_sum
